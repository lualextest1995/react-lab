...
export async function loader() {
  console.log("Test4 loader: 加载测试页面4数据");
  return { page: "test4", message: "这是测试页面 4" };
}

export default function Test4() {
  const data = useLoaderData<{ page: string; message: string }>();
  const { tabs, activeTabId, openTab, closeTab, switchTab, closeAll, closeOthers } =
    useTabContext();
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-8 p-8">
      <div className="max-w-3xl w-full border rounded-lg p-8 bg-gradient-to-br from-orange-50 to-amber-50 shadow-lg">
        <h1 className="text-4xl font-bold text-amber-700 mb-4">测试页面 4</h1>
        ...
        <div className="bg-white rounded-lg p-6 mb-6 shadow space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mb-3">
              所有對外暴露的方法都在此列出，可搭配瀏覽器主控台直接呼叫以手動操作 Tab 狀態。
            </p>
        ...
        <div className="flex gap-4">
          <Link to="/test3">
            <Button variant="outline">上一頁 (Test3)</Button>
          </Link>
          <Link to="/">
            <Button>返回首頁</Button>
          </Link>
